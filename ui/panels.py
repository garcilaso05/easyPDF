...
def build_right_panel(parent, app):
    ...
    # Formulario de edición
    form_frame = tk.LabelFrame(app.bookmarks_frame, text="Editar / Añadir marcador", padx=10, pady=10)
    form_frame.pack(fill="x", pady=10)

    tk.Label(form_frame, text="Título:").grid(row=0, column=0, sticky="w")
    app.title_entry = tk.Entry(form_frame, width=30)
    app.title_entry.grid(row=0, column=1, padx=5, pady=2)

    tk.Label(form_frame, text="Nivel (1=capítulo):").grid(row=1, column=0, sticky="w")
    ...
    tk.Button(btn_frame, text="➕ Añadir", command=app.add_bookmark, bg="#4CAF50", fg="white").pack(side="left", padx=2)
    tk.Button(btn_frame, text="✏️ Modificar", command=app.update_bookmark, bg="#2196F3", fg="white").pack(side="left", padx=2)
    tk.Button(btn_frame, text="🗑️ Eliminar", command=app.delete_bookmark, bg="#f44336", fg="white").pack(side="left", padx=2)
    ...
    tk.Button(zoom_frame, text="🔍−", command=app.zoom_out, width=4).pack(side="left", padx=2)
    app.zoom_label = tk.Label(zoom_frame, text="100%", width=6)
    app.zoom_label.pack(side="left", padx=5)
    tk.Button(zoom_frame, text="🔍+", command=app.zoom_in, width=4).pack(side="left", padx=2)
    ...
    # Sección de rotación
    rotation_frame = tk.LabelFrame(edit_controls_frame, text="🔄 Rotación", padx=10, pady=5)
    rotation_frame.pack(fill="x", pady=3, padx=5)

    rotation_btns = tk.Frame(rotation_frame)
    rotation_btns.pack()

    tk.Button(rotation_btns, text="↶ Izquierda",
    ...
    tk.Button(rotation_btns, text="↷ Derecha",
             command=lambda: app.rotate_page_right(app.current_page - 1) if app.current_page else None,
             bg="#2196F3", fg="white", width=10).pack(side="left", padx=3)

    # Sección de escala/tamaño
    scale_frame = tk.LabelFrame(edit_controls_frame, text="📐 Escala", padx=10, pady=5)
    ...
    app.scale_value_label.pack(side="left", padx=5)
    tk.Button(scale_info_frame, text="↺ Reset", command=app.reset_page_scale,
             bg="#607D8B", fg="white", font=("Arial", 8)).pack(side="right")

    # Sección de márgenes
    margin_frame = tk.LabelFrame(edit_controls_frame, text="📏 Márgenes (puntos)", padx=10, pady=5)
    ...
    tk.Button(margin_frame, text="↺ Quitar márgenes", command=app.reset_margins,
             bg="#607D8B", fg="white", font=("Arial", 8)).pack(pady=3)

    # Sección de blanco y negro
    bw_frame = tk.LabelFrame(edit_controls_frame, text="🎨 Color", padx=10, pady=5)
    ...
    tk.Label(preview_label_frame, text="📄 Original", font=("Arial", 9, "bold"),
             fg="#666").pack(side="left", expand=True)
    tk.Label(preview_label_frame, text="📄 Resultado", font=("Arial", 9, "bold"),
...
def build_right_panel(parent, app):
    ...
    tk.Label(right_frame, text="🌳 Árbol de marcadores", font=("Arial", 10, "bold")).pack(anchor="w")
